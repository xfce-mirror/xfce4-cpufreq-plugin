...
combo_changed (GtkWidget *combo, CpuFreqPluginConfigure *configure)
{
	...
	{
		cpuFreq->options->cpu_number = selected;
...
spinner_changed (GtkWidget *spinner, CpuFreqPluginConfigure *configure)
{
	...
	timeout *= 100;
	cpuFreq->options->monitor_timeout = timeout;
	cpu_freq_restart_monitor_timeout_handle ();
}

static void
cpu_freq_configure_response 	(GtkWidget *dialog,
				 int response,
				 CpuFreqPluginConfigure *configure)
{
	...
	cpu_freq_write_config (cpuFreq->plugin);

	g_free (configure);
}

void
cpu_freq_configure (XfcePanelPlugin *plugin, const char *tmp)
{
	...
	dialog = xfce_titled_dialog_new_with_buttons 
			(_("Configure CPU Frequency Monitor"),
		 	 NULL,
		 	 GTK_DIALOG_NO_SEPARATOR,
		 	 GTK_STOCK_CLOSE, GTK_RESPONSE_OK, NULL);
	xfce_titled_dialog_set_subtitle (XFCE_TITLED_DIALOG (dialog),
				_("Configure the cpu frequency plugin"));
	...
	spinner = configure->spinner_timeout = 
		gtk_spin_button_new_with_range (TIMEOUT_MIN, 
						TIMEOUT_MAX,
						TIMEOUT_STEP);
	gtk_spin_button_set_value (spinner, 
			(gdouble)cpuFreq->options->monitor_timeout / 100);
	...
	gtk_combo_box_set_active(GTK_COMBO_BOX(combo), cpuFreq->options->cpu_number);
	g_signal_connect(G_OBJECT(combo), "changed",
		G_CALLBACK(combo_changed), configure);


	/* check buttons for display widgets in panel */
	button = configure->display_icon = 
		gtk_check_button_new_with_mnemonic (_("Show CPU Icon"));
	gtk_box_pack_start (GTK_BOX (vbox), button, FALSE, FALSE, 0);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (button), 
			cpuFreq->options->show_icon);

	g_signal_connect (button, "toggled",
      		G_CALLBACK (check_button_changed), configure);

	button = configure->display_freq = 
		gtk_check_button_new_with_mnemonic (_("Show CPU Frequency"));
	gtk_box_pack_start (GTK_BOX (vbox), button, FALSE, FALSE, 0);
	gtk_toggle_button_set_active (GTK_TOGGLE_BUTTON (button), 
			cpuFreq->options->show_label_freq);
	...
	g_signal_connect(dialog, "response",
			G_CALLBACK(cpu_freq_configure_response), configure);
